...
exports.run = (client, message, args, level) => {
  ...
  if (!args[0]) {
    ...
    // I hate template literal multiline strings. Disgusting.
    let output = "- Requesting feedback requires a point to spend.\n";
    output += "- Points are gained by giving feedback.\n\n";
    output += `Type ${prefix}${help.name} to see the commands.\n`;
...
const commandsHelpOutput = (client, message, level) => {
	...
	if (cat === "Feedback")
	  output += `${settings.prefix[0]}${x.help.name}${" ".repeat(longest - x.help.name.length)} :: ${x.help.description}\n`;
    });
