...
exports.run = (client, message) => {
	...
	const onlineUsers = message.guild.members.filter(member => member.presence.status === "online" && !member.user.bot)
		.size;
	const awayUsers = message.guild.members.filter(member => member.presence.status === "idle" && !member.user.bot).size;
	const offlineUsers = message.guild.members.filter(member => member.presence.status === "offline" && !member.user.bot)
		.size;
	const dndUsers = message.guild.members.filter(member => member.presence.status === "dnd" && !member.user.bot).size;
	...
	const buildEmbed = new Discord.RichEmbed()
		...
		.setColor(0x00ae86)
		.setDescription(
			`${onlineEmoji}Online Members: ${onlineUsers}\n\
